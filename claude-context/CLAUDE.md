...
This repo (`coverage-guided-concolic-pipeline`) is the orchestrator for the full test generation pipeline. It does **not** contain the pathcov or JDart Java source code — those live in sibling repos. This project only holds Docker image definitions, shell/Python scripts for orchestration, and configuration templates.
...
├── scripts/
│   ├── generate_sut_configs.py      # Reads sut.yml → generates pathcov/configs/sut.config + jdart/configs/sut_gen.jpf
...
│   ├── jdart_format_classpath.py    # Formats classpath entries for JDart's .jpf config (semicolon-separated, backslash-continued)
│   └── compose_up.sh               # Convenience: starts containers in dev mode without running the pipeline
├── docker-compose.yml               # Base compose: defines pathcov + jdart services, pipeline-data volume
...
├── jdart/
│   ├── Dockerfile                   # Image: Ubuntu 14.04 + Java 8 + jpf-core + jConstraints + Z3 4.4.1 + JDart
│   └── configs/
│       ├── jdart.jpf                # Default JDart settings (CoverageHeuristicStrategy, z3, timeouts, test gen)
...
│       └── coverage_heuristic.config  # Points JDart to /data/blockmaps/icfg_block_map.json
├── block-diff/
...
2. Runs `scripts/generate_sut_configs.py` — parses `configs/sut.yml` + `sut.env` + `container.env` to generate:
   ...
   - `jdart/configs/sut_gen.jpf` (JPF properties with classpath, target, concolic.method)
...
7. Runs **JDart stage**: `compose_exec jdart /jdart-project/jpf-core/bin/jpf /configs/sut.jpf`

### Docker volume architecture

Both containers share a Docker named volume `pipeline-data` mounted at `/data`. The pathcov stage writes the coverage block map to `/data/blockmaps/icfg_block_map.json`, and the JDart stage reads it from there (via `coverage_heuristic.config`).
...
| Mount point   | pathcov                          | jdart                            |
|---------------|----------------------------------|----------------------------------|
| `/data`       | pipeline-data volume (shared)    | pipeline-data volume (shared)    |
| `/configs`    | `./pathcov/configs` (ro)         | `./jdart/configs` (ro)           |
...
2. Clones and `mvn install`s **intellij-coverage-model** (v3.0.1) — shared DTO for coverage data (also used by JDart)
...
4. **`generate_block_map`** — Runs pathcov's `GenerateBlockMap` which uses SootUp to build the interprocedural CFG of the target method, then maps coverage data (line-level) onto CFG blocks. Each block gets a structural hash. Outputs `$DATA_DIR/blockmaps/icfg_block_map.json`. **This is the key artifact** — JDart reads this to guide exploration.
...
2. Clones and builds **jpf-core** (Java PathFinder, tag `JPF-8.0`) with `ant` — the bytecode VM that JDart runs on top of
...
6. Configures JPF: writes `/root/.jpf/site.properties` pointing to jpf-core and jdart
7. Configures jConstraints: copies z3 JARs into `/root/.jconstraints/extensions/`
8. Clones **JDart** (forked, from `SonnyZ789123/jdart`) and builds with `ant`

The image tag is `sonnyz789123/jdart-image:<version>`. JDart has no build manager — it uses ant and manual classpath management.

### How JDart is invoked

From `run_pipeline.sh`:
```bash
compose_exec jdart /jdart-project/jpf-core/bin/jpf /configs/sut.jpf
```

The `jpf` launcher script starts Java PathFinder with the given config file. JPF loads JDart as a shell plugin.

### Config chain

`sut.jpf` (user-editable, bind-mounted at `/configs/sut.jpf`) includes:
1. **`jdart.jpf`** — Default JDart settings:
   - `shell=gov.nasa.jpf.jdart.JDart` (loads JDart as the JPF shell)
...
**`coverage_heuristic.config`** tells JDart where to find the block map produced by pathcov:
...
In dev mode, `docker-compose.override.yml` bind-mounts the local jdart source (`../../jdart/`) over `/jdart-project/jdart`. To recompile after changes, exec into the container and run `ant`:
```bash
docker exec -it jdart /bin/bash
cd /jdart-project/jdart && ant
```
This is necessary because the host machine likely has JDK 21+, but JDart requires JDK 8.